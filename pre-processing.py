...
#
# # need to change amount format before running the for loop to add incoming outgoing
# array = []
# for i in range(0, len(anz)):
#     x = anz.iloc[i]  # grab the first row
#     x = x['Amount']
...
print(np.shape(sc_credit))
print(np.shape(sc_check))
print(np.shape(sc_sav))
print(np.shape(cbl_credit))
print(np.shape(cbl_checking))

combine = pd.concat([sc_sav, sc_check, sc_credit, cbl_credit, cbl_checking]).reset_index(drop=True)
