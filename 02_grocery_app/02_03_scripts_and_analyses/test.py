...
#
#     e1_entry = ttk.OptionMenu(top, e1, *bfast)
...
#
#     e4_entry = ttk.OptionMenu(top, e4, *bfast)
...
#
#     e7_entry = ttk.OptionMenu(top, e7, *bfast)
...
#
#     e10_entry = ttk.OptionMenu(top, e10, *bfast)
...
#
#     e13_entry = ttk.OptionMenu(top, e13, *bfast)
...
#
#     e16_entry = ttk.OptionMenu(top, e16, *bfast)
...
#         # recipes = (np.unique(df['Recipe_Title']))  # grab each unique recipe from the recipe book and print it so the
#         """
...
#
#         cleaned_data = pd.DataFrame(
#             {"Recipe_Title": array1, "Ingredient": duplicates_list, "Type": type_new, "Quantity": array2})
#         # cleaned_data.to_excel('cleaned.xlsx')
#
#         others = x.loc[(x['Duplicate_search'] == False)]  # all the ones where the original dup search was false.
#         final_list = pd.concat([cleaned_data, others])
...
#
#     myButton = Button(top, text="Run", command=executeList, fg='blue')
